...
|---|---|---|---|---|
| 2026-04-14 | Claude Code | **Security hardening (full audit):** 10 commits closing all critical/high/medium vulnerabilities. Path traversal guard in `testerFileService` (`guardPath`). JWT auth on all 6 tester endpoints (`/api/config`, `/api/prompts`, `/api/cases`, `/api/results`, `/api/run`, `/api/chat`). `WRITABLE_SETTINGS` allowlist in `PUT /api/settings` (blocks arbitrary key injection). Manychat returns 503 when `WEBHOOK_SECRET` not configured. Login timing oracle fixed (`DUMMY_HASH` constant). `name` maxLength 100 on register. JWT cookie maxAge reduced 30→7 days. `leadHash` includes `source` prefix. `timingSafeEqual` in `verifyMetaChallenge`. Shared `requireAuth` utility. 108/108 tests passing. | commits `074bf6c`–`3c189cf` · `docs/superpowers/plans/2026-04-14-security-fixes.md` | Re-hash migration for leads with old `external:<id>` hash format (see CLAUDE.md tech debt). E2E coverage for setup-2, done screen, settings panel. |
| 2026-04-14 | Claude Code | **Engineering cleanup + bug fixes:** file structure reorganized (`controllers/` deleted, `integrations/llm/` isolated per provider, `app/plugins.ts` extracted, `types/errors.ts` AppError hierarchy, `routing/config.ts`). TTL cache on `settingsService`. Explicit DI on `LeadIngestionService`. `createApp.ts` reduced to ~40 lines with global error handler. **Bugs fixed:** `askAnthropic` now uses passed `apiKey`/`model`; LLM providers guard array access against empty/unexpected responses; `authService.hashPassword` redundant `AuthError` removed; setup-2 inputs trimmed before save; `btn-test-llm` disabled when key empty; `btn-finish` disabled during fetch; Gemini models updated to `gemini-2.5-flash`/`gemini-2.5-pro`. 77 tests passing. | commits `5052973`–`9f7d743` | E2E coverage for setup-2, done screen, settings panel |
...
| Priority | Item | Notes |
|---|---|---|
| 🔴 High | leadHash re-hash migration | Leads stored with old `external:<id>` format may dedup incorrectly. Run `SELECT lead_hash FROM leads WHERE lead_hash LIKE 'external:%' AND lead_hash NOT LIKE 'external:%:%'` to assess scope before deploying. |
...
| 🟡 Medium | `GET /metrics` sem autenticação | Em produção, expõe volume de tráfego por rota. Restringir por IP no load balancer ou adicionar Basic Auth. |
